...
local function giveStaffSpawnTools(ply)
    if not IsValid(ply) or not ply:IsPlayer() then
        return
    end

    if not ZCityAftermath or not ZCityAftermath.CanUseStaffMenus or not ZCityAftermath.CanUseStaffMenus(ply) then
        return
    end
    ...
    if not istable(RPExtraTeams[teamId]) then
        return
    end
...
local function findFallbackDarkRPTeam()
    if GAMEMODE and GAMEMODE.DefaultTeam and RPExtraTeams and istable(RPExtraTeams[GAMEMODE.DefaultTeam]) then
        return GAMEMODE.DefaultTeam
    end

    for teamId, teamData in pairs(RPExtraTeams or {}) do
        if teamId ~= TEAM_SPECTATOR and teamId ~= TEAM_UNASSIGNED and istable(teamData) then
            return teamId
        end
    end
end

local function restoreDarkRPTeamState(ply)
    if not IsValid(ply) or not ply:IsPlayer() then
        return false
    end

    if not ZCityAftermath or not ZCityAftermath.UsesDarkRPBase or not RPExtraTeams then
        return false
    end

    local currentTeam = ply:Team()
    if currentTeam ~= TEAM_SPECTATOR and currentTeam ~= TEAM_UNASSIGNED and istable(RPExtraTeams[currentTeam]) then
        return true
    end

    local fallbackTeam = findFallbackDarkRPTeam()
    if not fallbackTeam or not istable(RPExtraTeams[fallbackTeam]) then
        return false
    end

    local targetTeam = RPExtraTeams[fallbackTeam]
    local restorePos = ply.ZCityAftermathLastValidPos
    local restoreAng = ply.ZCityAftermathLastValidAng

    ply:UnSpectate()
    ply:SetMoveType(MOVETYPE_WALK)
    ply:SetTeam(fallbackTeam)

    if ply.updateJob then
        ply:updateJob(targetTeam.name or "Citizen")
    elseif ply.setDarkRPVar then
        ply:setDarkRPVar("job", targetTeam.name or "Citizen")
    end

    if ply.setSelfDarkRPVar then
        ply:setSelfDarkRPVar("salary", tonumber(targetTeam.salary) or 45)
    end

    if ply.setDarkRPVar then
        ply:setDarkRPVar("HasGunlicense", targetTeam.hasLicense == true)
    end

    if isvector(restorePos) then
        timer.Simple(0, function()
            if not IsValid(ply) then
                return
            end

            ply:SetPos(restorePos + Vector(0, 0, 4))

            if isangle(restoreAng) then
                ply:SetEyeAngles(Angle(0, restoreAng.y or 0, 0))
            end
        end)
    end
...
if ZCityAftermath and ZCityAftermath.UsesDarkRPBase then
    hook.Add("Think", "ZCityAftermath.TrackLastValidPosition", function()
        if not RPExtraTeams then
            return
        end
        ...
        for _, ply in ipairs(player.GetAll()) do
            ...
            ply.ZCityAftermathLastValidPos = ply:GetPos()
            ply.ZCityAftermathLastValidAng = ply:EyeAngles()
        end
    end)

    hook.Add("PlayerInitialSpawn", "ZCityAftermath.RecoverDarkRPTeam", function(ply)
        timer.Simple(0, function()
            if not IsValid(ply) then
                return
            end

            restoreDarkRPTeamState(ply)
        end)
    end)

    hook.Add("PlayerSpawn", "ZCityAftermath.StaffSpawnTools", function(ply)
        timer.Simple(0, function()
            if not IsValid(ply) then
                return
            end
