...
local darkRPRoundStub = setmetatable({
    name = "darkrp",
    start_time = 0,
    CanSpawn = function()
        return true
    end,
    GetTeamSpawn = function()
        local spawn = (zb.GetRandomSpawn and zb:GetRandomSpawn()) or Vector(0, 0, 0)
        return { spawn }, { spawn }
    end
}, {
    __index = function()
...
function CurrentRound()
    ...
    if round and zb.modes[round] then
        return zb.modes[round], zb.CROUND
    end

	return darkRPRoundStub, zb.CROUND or "darkrp"
end
