...
ZCityAftermath = ZCityAftermath or {}
ZCityAftermath.UsesDarkRPBase = true

local menuAccessGroups = {
	admin = true,
	superadmin = true
}

function ZCityAftermath.CanUseStaffMenus(ply)
	if not IsValid(ply) then
		return false
	end

	if not ply.GetUserGroup then
		return false
	end

	local group = string.lower(ply:GetUserGroup() or "")
	return menuAccessGroups[group] == true
end
...
local spawn = {"PlayerGiveSWEP", "PlayerSpawnEffect", "PlayerSpawnNPC", "PlayerSpawnObject", "PlayerSpawnProp", "PlayerSpawnRagdoll", "PlayerSpawnSENT", "PlayerSpawnSWEP"}
...
if CLIENT then
	local function canUseSpawnMenu(ply)
		return ZCityAftermath.CanUseStaffMenus(ply)
	end
	...
	hook.Add( "SpawnMenuOpen", "SpawnMenuWhitelist", function()
		...
		if canUseSpawnMenu(ply) then return end
		return false
	end )

	hook.Add("PlayerBindPress", "ZCityAftermath.RestrictHelpMenu", function(_, bind, pressed)
		if not pressed then
			return
		end

		bind = string.lower(bind or "")
		if not string.find(bind, "gm_showhelp", 1, true) then
			return
		end

		if ZCityAftermath.CanUseStaffMenus(LocalPlayer()) then
			return
		end
	...
	hook.Add("ContextMenuOpen", "ZCityAftermath.RestrictContextMenu", function()
		if ZCityAftermath.CanUseStaffMenus(LocalPlayer()) then
			return
		end
...
if SERVER then
	function GM:ShowHelp(ply)
		if not ZCityAftermath.CanUseStaffMenus(ply) then
			return
		end

		if self.BaseClass and self.BaseClass.ShowHelp then
			return self.BaseClass.ShowHelp(self, ply)
		end
	end
end
